...
function ValidateForm(form)
{
	...
	inputs = [];
	day = selectCreateCall($('#dobDay'),'day');
	month = selectCreateCall($('#dobMonth'),'month');
	year = selectCreateCall($('#dobYear'),'year');
	...
	function active(obj)
	{
		...
		focusIcon.src = 'assets/img/loader_grey.gif';
	...
	function valid(obj)
	{
		...
		$(obj).parent().find('span').addClass('icon-checkmark2').css('color','#3498db');
	...
	function formSubmit()
	{
		$('button').on('click',function(event)
		{
			...
			else
			{
				...
				$('html, body').animate({
					scrollTop: $('#scrollTo').offset().top -20}, 800
				)	
  			}
